...
export const useAxiosInstanceCM = () => {
  const navigate = useNavigate()
  const location = useLocation()
  const { config } = useConfigStore.getState()
  const baseURL = config?.API_URL_CM || 'https://cm.api.dmbapp.com'
  ...
  instance.interceptors.request.use(
    ...
    (error) => {
      const pageInfo = getCurrentPageInfo()
      LoggerService.error('AxiosInstanceCM.Request', 'Request interceptor error', {
        error: error.message,
        meta: { page: pageInfo }
      })
  ...
  instance.interceptors.response.use(
    (response) => {
      ...
      LoggerService.info('AxiosInstanceCM.Response', `Success ${method} ${fullUrl}`, {
    ...
    async (error) => {
      ...
      // ── 401 early-exit ────────────────────────────────────────────────────
      if (status === 401) {
        LoggerService.warn('AxiosInstanceCM.Auth', 'Unauthorized access - redirecting to login', {
      ...
      if (status === 403) {
        LoggerService.warn('AxiosInstanceCM.Auth', 'Forbidden access - user lacks permission', {
          url: fullUrl,
          method,
        ...
        LoggerService.error('AxiosInstanceCM.Server', 'Internal server error', {
          url: fullUrl,
        ...
        LoggerService.error('AxiosInstanceCM.Network', 'Network error - no response from server', {
          url: fullUrl,
          error: error.message,
          meta: { page: pageInfo }
        })
        ...
        LoggerService.warn(
          'AxiosInstanceCM.HTTP',
          `HTTP Error ${status} for ${method} ${fullUrl}`,
          {
            request: { url: fullUrl, method },
            response: error.response
...
export const createStandaloneAxiosCM = () => {
  const { config } = useConfigStore.getState()
  const baseURL = config?.API_URL_CM || 'https://cm.api.dmbapp.com'
  const token = localStorage.getItem('token')
  ...
  instance.interceptors.request.use(
    ...
    (error) => {
      console.error('[StandaloneAxiosCM] Request error:', error.message)
      return Promise.reject(error)
    }
  )
  ...
  instance.interceptors.response.use(
    ...
    (error) => {
      ...
      console.error(
        `[StandaloneAxiosCM] Error ${method} ${fullUrl} - Status: ${status || 'No Response'}`
      )

      if (status === 401) {
        console.warn('[StandaloneAxiosCM] Unauthorized - token may be expired')
      }
