...
import { useReportIncomeCustomer } from './hook/useReportIncomeCustomer'
import Breadcrumb from '@renderer/components/ui/breadcrumb/Breadcrumb'

export const ReportIncomeCustomerPage = () => {
  const report = useReportIncomeCustomer()
  const {
    title,
    subtitle,
    breadcrumbs,
              ...
              <TableContainer>
                <Table>
                  ...
                  <TableBody>
                    {data.map((row, index) => (
                      <TableRow
                        key={row.guid || row.id || `${row.customer_name || 'row'}-${index}`}
                        hover
                      >
                        {columns.map((column) => (
                          <TableCell key={column.id} align={column.align || 'left'}>
