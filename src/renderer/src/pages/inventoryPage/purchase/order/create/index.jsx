  ...
  return (
    <Box>
      ...
      <Breadcrumb
        ...
        items={[{ to: '/', title: 'Home' }, { title: 'inventory' }, { title: 'Buat Pembelian' }]}
      />
            ...
            <Box bgcolor={'white'} p={3}>
              ...
              >
                <Autocomplete
                  sx={{ width: '250px' }}
                  options={preOrders}
                  loading={loading.fetchPreOrders}
                  getOptionLabel={(option) => option.nomor || ''}
                  value={preOrders.nomor}
                  onChange={(event, newValue) => {
                    console.log(newValue, 'isi preorder')
                    if (newValue == null) {
                      setSelectedPreOrder(null)
                      setFormData((prevFormData) => ({
                        ...prevFormData,
                        items: [{ product_id: '', qty: 0, satuan: '', notes: '' }]
                      }))
                      ...
                      setSelectedPreOrder(newValue)
                      newValue.items.forEach((newV, index) => {
                        handleOrderChange(index, 'product_id', newV ? newV.guid : '')
                        handleOrderChange(index, 'qty', newV.qty)
                        handleOrderChange(index, 'satuan', newV ? (newV.satuan ?? '') : '')
                        handleOrderChange(index, 'notes', newV ? (newV.notes ?? '') : '')
                      })
                      setFormData((prevData) => ({
                        ...prevData,
                        pre_order_id: newValue.id
                      }))
                    }
                  }}
                  renderInput={(params) => (
                ...
                <Grid size={{ xs: 12, sm: 5 }}>
                  <Box textAlign="left">
                    ...
                    <Autocomplete
                      ...
                      renderInput={(params) => (
                        <CustomTextField {...params} placeholder="Pilih Supplier" fullWidth />
                      )}
                    />
                  </Box>
                </Grid>
              ...
              >
                ...
                <Button
                  ...
                  variant="contained"
                  color="primary"
                  disabled={selectedPreOrder != null}
                      ...
                      {formData.items.length > 0 ? (
                        formData.items.map((order, index) => (
                          <TableRow key={index}>
                              ...
                              <Autocomplete
                                ...
                                renderInput={(params) => (
                                  <CustomTextField
                                    ...
                                    InputProps={{
                                      ...params.InputProps,
                                      readOnly: selectedPreOrder != null
                                    }}
                                  />
                                )}
                              />
                            ...
                            <TableCell>
                              <Typography>{formatRupiah(order.unitTotalPrice)}</Typography>
                            </TableCell>
                            <TableCell>
