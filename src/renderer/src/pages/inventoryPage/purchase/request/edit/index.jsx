...
import { UseUpdatePurchaseRequest } from './hook/useUpdatePurchaseRequest'
...
export const UpdatePurchaseRequestPage = () => {
                          ...
                          <TableRow key={index}>
                              ...
                              <Autocomplete
                                ...
                                options={[
                                  ...products.filter(
                                    (transaction) =>
                                      !formData.items.some(
                                        (o, i) => o.product_id === transaction.guid && i !== index
                                      )
                                  ),
                                  ...formData.items.filter((order) => order.product_id) // Include existing orders
                                ].filter(
                                  (option) => option.guid !== formData.items[index]?.product_id
                                ...
                                value={
                                  products.find(
                                    (t) => t.guid === formData.items[index]?.product_id
                                  ) || formData.items[index] // Fallback to current order if not found in transactions
                                }
                                onChange={(event, newValue) => {
                                  ...
                                  handleOrderChange(index, 'qty', 1) // Set item name
                                  handleOrderChange(
                                    index,
                                    'satuan',
                            ...
                            <TableCell colSpan={1} style={{ padding: isMobile ? '0px' : '16px' }}>
                              <CustomTextField
                                ...
                                fullWidth={isMobile} // Apply fullWidth on mobile screens
                                style={{
                                  width: isMobile ? '100%' : 'auto' // Adjust width on mobile and desktop
                                }}
                              />
                            </TableCell>
                            <TableCell>
                              <Autocomplete
                                ...
                                options={units.filter(
                                  (transaction) =>
                                    !formData.items.some(
                                      (o, i) => o.satuan === transaction.satuan && i !== index
                                    )
                                )}
                                getOptionLabel={(option) => option.name || ''} // Adjust according to your data
                                value={
                                  units.find((t) => t.name === formData.items[index]?.satuan) ||
                                  formData.items[index].satuan // Fallback to current order if not found in transactions
                                }
                                onChange={(event, newValue) => {
                              ...
                              {/* <CustomTextField
                                                    ...
                                                    placeholder="Unit"
                                                    onChange={(e) =>
                                                        handleOrderChange(index, "satuan", e.target.value)
                                                    }
                                                    fullWidth
                                                /> */}
                            </TableCell>
